...
export default function StudentsList() {
  ...
  const { items: rows, total, loading, loadingMore, hasMore, sentinelRef } = useInfiniteList<StudentDirectoryRow>(
    (page) => {
      ...
      if (bloodGroupId) qs.set('bloodGroupId', bloodGroupId)
      return api.get<PagedResult<StudentDirectoryRow>>(`/api/students?${qs.toString()}`)
    },
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          {loading ? 'Searching...' : `${total.toLocaleString()} currently enrolled — view-only directory.`}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <div className="relative flex-1 min-w-[220px] max-w-xs">
          ...
          <Input placeholder="Search by name..." className="pl-9 pr-9" value={q} onChange={(e) => setQ(e.target.value)} />
